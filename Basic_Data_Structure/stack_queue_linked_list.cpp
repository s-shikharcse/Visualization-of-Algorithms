...
{   //maximum no. of nodes can be 5
	...
	line(44,393,69,393); //creating arrow for head '->'
	line(64,388,69,393);
	line(64,398,69,393);
	outtextxy(73,390,"NULL");// initially Head->NULL 
	size=0;
    }
    nodeptr create();
    void add(); 
...
void list::add()//add a node at the begining
{
    if(size<5)
    {
    nodeptr new1;
    new1=new node;
    setcolor(10); //taking input in double green rect
    ...
    new1->link=head->link;
    head->link=new1;
    rectangle(70,285,150,335);//creating node in form of rectangle
    line(130,285,130,335);// partitioning node for storing address|____|_|
    gotoxy(12,20);
    cout<<new1->data;
    size++;
    delay(500);
    ...
    for(int j=70;j<170;) //shifting the already contained node forward to make space for new one
    {
    ...
    for(int i=285;i<=365;) //arranging new node at right pos
    {
    ...
    setcolor(BLACK);//removing the input window
    rectangle(288,98,602,152);
    rectangle(290,100,600,150);
    gotoxy(39,8);
    cout<<"                                          ";
...
void list::add(char end) //add a node at the end
{
    if(size<5)
    {
    nodeptr new1;
    ...
    temp->link=new1;
    new1->link=NULL;
    //creating node(rect) for storing value
    ...
    int area;
    char *buff;  
    area=imagesize(70+(size*100),365,525,416); //putting the new node at right pos
    buff=new char(area);
...
void list::add(int x) //add a node in between
{
    if(size<5)
    {
      nodeptr new1;
...
void list::del()//delete a node from specified position
{
  if(head->link!=NULL)
  {
    nodeptr temp,temp1;
...
class stack1
{
     int max,flag; // max for maximum size of stack
				   // flag is used to know whether stack created or not?	
     protected:
	 float a[10];  // array to store the values in stack
...
int stack1::create()
{
	 ...
	 {
	    outtextxy(50,275,"Do you want to create new stack(Y/N)");
	    rectangle(500,400-(30*max),575,400); //(500,400)coordinates of left diagonal lower point 
	    for(int i=1;i<max;i++)
		line(500,400-(30*i),575,400-(30*i)); //creating partitions in stack(rectangle)
	    outtextxy(400,413-(top*30)-30,"STACK TOP ->    -1");
	    outtextxy(400,413,"STACK TOP ->    -1");
	    int area;
	    char *buff;
	    area=imagesize(510,15,565,30); //15 to 30 as max size of stack is 15
	    buff=new char[area];
	    for(;top>=0;top--)
	    {
     ...
     setcolor(10);//light green
     rectangle(40,260,350,300); //that double boundary rectangle 
     rectangle(38,258,352,302); // where input is given
     setcolor(WHITE); // for input
     ...
     flag=1; // stack is created
     top=-1;
     setcolor(WHITE); //for stack
     
     /*creating rectangle for stack representation*/
     ...
     //creating partition within stack(rect)
     //500 to 575 is the width of the rectangle
     for( i=1;i<max;i++)
...
if(flag==1)
{
	...
	getch();
	setcolor(BLACK); // to remove above printed value
 	outtextxy(50,270,"Stack is full ");
	setcolor(WHITE);
    }
    else
    {
    // taking input in that green box
     setcolor(10);
     ...
     cin>>a[++top];
     int j=1;
     // to show value going into stack
...
// Stack class inherited in public mode
...
//function to caluculate to priority of operator
...
// imp function to caluculate postfix expression
void post :: postfix(char infix[],char postr[])
{
    ...
    int outpos=0;
    char topsym,sym;
    create(10); //inherited function of stack class
...
/*function for postfix evaluation
  inherited from post class,parent 
  class is stack class*/
...
float evaluate::eval(char expr[])
{
    ...
    create(10); // function inherited from grandparent class
...
{
    cleardevice();
    post p; // creating object of post class
    ...
    outtextxy(75,175,"to postfix and to evaluate a postfix expression");
    outtextxy(75,200,"You are required to enter an infix expression");
    outtextxy(75,225,"Note : You have to enter operands of single digits");
    ...
    cleardevice();
    setcolor(YELLOW);
    rectangle(0,0,getmaxx(),getmaxy()); // for boundary of window
    rectangle(2,2,getmaxx()-2,getmaxy()-2); //double boundary of window
    setcolor(WHITE);
    gotoxy(4,2);
    textcolor(WHITE);
    ...
    char ch=getch();
    evaluate e; //object of evaluate class
    if(ch=='Y'||ch=='y')
    {
	cleardevice();
	setcolor(YELLOW);
...
class queue1
{
    int a[10],front,rear,flag,max; // array a to store the q element
    							   // max q size is 10	
    public:
    void add();
    void del();
...
int queue1 :: create()    
{
     gotoxy(24,17);
     setcolor(10); // light green
     if(flag==1)  // if q already created
     {   setcolor(WHITE);
     ...
     setcolor(10); //taking input within double green rect
     ...
     settextstyle(0,0,1);
     setcolor(BLACK);
     rectangle(190,260,500,300);//removing that green box of input
     rectangle(188,258,502,302);
     outtextxy(200,275,"Enter the size of queue :  _");
     gotoxy(53,18);
     cout<<"        ";
...
void queue1::del()
{
	...
	else
	{
	    ...
	    int area;
	    int j=1;
	    setcolor(10); // green box for input
	    rectangle(190,260,510,300);
	    rectangle(188,258,512,302);
	    setcolor(WHITE);
	    ...
	    j=1;
	    for(int i=390;i>355;i--) //for showing opening of q
	    {
		putpixel(50,i,BLACK);
...
void funct(char *C1,char *C2,char *C3,char *C4,char *title,int d,int g)
{
    ...
    gotoxy(20,8);
		...
		setcolor(10);
		rectangle(0,0,maxx,maxy);// boundary rectangle
		rectangle(2,2,maxx-2,maxy-2);//" "
		rectangle(40+d,80,250+d,230);//inner rectangle showing options
		rectangle(38+d,78,252+d,232);// " "
		line(40+d,205,250+d,205);
		setcolor(WHITE);
		settextstyle(0,0,1);
		outtextxy(50+d,100,C1);//using macro defined above
...
void main()
{
    ...
    maxx=getmaxx();
    maxy=getmaxy();
    rectangle(0,0,maxx,maxy);//white color boundary rectangle
    setcolor(11); 
    settextstyle(7,0,1);
    delay(1000);
...
outtextxy(maxx/2-textwidth("IMPLEMENTATION")/2,50,"IMPLEMENTATION");
    ...
    outtextxy(maxx/2-textwidth("DATA  STRUCTURES")/2,150,"DATA STRUCTURES");
    delay(1500);
    setcolor(YELLOW);
    settextstyle(7,0,2);
	...
	do 
	{
		...
		while(x!=13)
		{
			...
			if(x==80) // this is used to store the value pointed
				ch=(ch==5?1:ch+1);//by key and print the optionb selected
			else
			   if(x==72)
				ch=(ch==1?5:ch-1);
			setcolor(10);
		...
		switch(ch)
		{
			case 1:funct(S1,S2,S3,S4,S,0,0); //for stack
			       break;
			case 2:funct3(); // for stack implementation
			       break;
			case 3:funct(Q1,Q2,Q3,Q4,Q,150,19); //for queue
			       break;
			case 4://closegraph();
			       funct1();
			       break;
    ...
    setcolor(YELLOW);
    outtextxy(150,300,"Shikhar Sharma");
    getch();
    closegraph();
    restorecrtmode();
    //return(0);
